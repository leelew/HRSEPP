...
def main(id):
    ...
    x_train_dec = np.load('/hard/lilu/z_train_{}.npy'.format(id))

    x_valid = np.load('/hard/lilu/x_valid_{}.npy'.format(id))
    y_valid = np.load('/hard/lilu/y_valid_{}.npy'.format(id))
    x_valid_dec = np.load('/hard/lilu/z_valid_{}.npy'.format(id))

    x_test = np.load('/hard/lilu/x_test_{}.npy'.format(id))
    y_test = np.load('/hard/lilu/y_test_{}.npy'.format(id))
    x_test_dec = np.load('/hard/lilu/z_test_{}.npy'.format(id))

    print('1')
    print(x_train_dec.shape)
    x_train, y_train, x_train_dec = DataLoader(len_input=1,
                                  len_output=1,
                                  window_size=2,
                                  use_lag_y=True,
                                  mode='train')(x_train, y_train, x_train_dec)

    x_valid, y_valid, x_valid_dec = DataLoader(len_input=1,
                                  len_output=1,
                                  window_size=2,
                                  use_lag_y=True,
                                  mode='valid')(x_valid, y_valid, x_valid_dec)

    x_test, y_test, x_test_dec = DataLoader(len_input=1,
                                len_output=1,
                                window_size=2,
                                use_lag_y=True,
                                mode='valid')(x_test, y_test, x_test_dec)
    print(x_train_dec.shape)
    # train
    ...
    train(x_train,
          ...
          n_forecast_months=1,
          n_output_classes=1,
          model_name='convlstm',#'causal',#'seq2seq',
