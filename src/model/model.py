...
class ForestCausalLSTM(tf.keras.Model):
    ...
    def __init__(
        ...
        self.num_tree = len(group_num_child_nodes.keys())

        self.group_clstm = []
        for timestep in self.group_num_child_nodes.keys():
    ...
    def __call__(self, inputs):
        ...
        out = []
        for i, timestep in enumerate(group_num_child_nodes.keys()):
            out.append(self.group_clstm[i](inputs))


        out = self.dense(y_pred_clstm)
