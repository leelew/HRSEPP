...
def preprocess_raw_smap_forcing(input_path,
    ...
    for date in dates:
        ...
        if os.path.exists(out_path + filename):
            ...
            # get shape
            _, force, _, _, _, Nlat, Nlon = read_single_smap(l[0],
                                                      lat_lower, lat_upper,
                                                      lon_left, lon_right)

            # integrate from 3-hour to daily
            ssm_dd = np.full((Nlat, Nlon, force.shape[-1], len(l)), np.nan)

            for i, path in enumerate(l):
                _, ssm_dd[:, :, i], lat_matrix, lon_matrix, _, _, _ = \
            ...
            f.createDimension('feature', size=ssm_dd.shape[-1])


            lon = f.createVariable('longitude', 'f4', dimensions='longitude')
            lat = f.createVariable('latitude', 'f4', dimensions='latitude')
            force = f.createVariable(
                'force', 'f4', dimensions=('latitude', 'longitude', 'feature'))
...
def preprocess_train_daily_smap_forcing(input_path,
    ...
    force, lat_, lon_ = read_daily_smap_force(input_path,
                                                 begin_date, end_date)
...
def preprocess_test_daily_smap_forcing(input_path,
    ...
    force, lat_, lon_ = read_daily_smap_force(
        input_path, begin_date, end_date)

    # preprocess test data
    force, min_, max_ = preprocess_test_daily_data(
        force, input_preprocess_path)
